...
export function searchMedikamente({
  ...
  if (query) {
    conditions.push(
      '(m.medikamentenname LIKE ? OR m.wirkstoff LIKE ? OR m.atc_code LIKE ? OR m.hersteller LIKE ? OR m.pzn LIKE ?)'
    );
...
export function getFilterOptions(filterType: string, callback: (options: string[]) => void) {
  ...
  switch (filterType) {
    ...
    case 'darreichungsform':
      sql =
        'SELECT DISTINCT darreichungsform FROM medikamente WHERE darreichungsform IS NOT NULL ORDER BY darreichungsform';
      break;
    case 'atc_code':
      sql =
        'SELECT DISTINCT atc_code FROM medikamente WHERE atc_code IS NOT NULL ORDER BY atc_code';
      break;
    case 'nebenwirkung':
    ...
    case 'hersteller':
      sql =
        'SELECT DISTINCT hersteller FROM medikamente WHERE hersteller IS NOT NULL ORDER BY hersteller';
      break;
    default:
      callback([]);
      return;
