...
const INPUT_FILE = path.join(
  __dirname,
  '../../icd10gm2025syst-claml/Klassifikationsdateien/icd10gm2025syst_claml_20240913.xml'
);
...
async function parseICD10GM() {
  ...
  try {
    ...
    console.log(
      `Parsing abgeschlossen. ${icdCodes.length} ICD-Codes wurden in ${OUTPUT_FILE} gespeichert.`
    );
  } catch (error) {
...
function getFachgebietFromICDCode(code) {
  ...
  switch (firstChar) {
    case 'A':
    case 'B':
      return 'Infektiologie';
    case 'C':
    case 'D':
      return firstChar === 'C' || code.startsWith('D0') ? 'Onkologie' : 'Hämatologie';
    case 'E':
      return 'Endokrinologie';
    case 'F':
      return 'Psychiatrie';
    case 'G':
      return 'Neurologie';
    case 'H':
      return code.startsWith('H0') || code.startsWith('H1') || code.startsWith('H2')
        ? 'Augenheilkunde'
        : 'HNO';
    case 'I':
      return 'Kardiologie';
