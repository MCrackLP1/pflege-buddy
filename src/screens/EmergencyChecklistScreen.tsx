...
const EmergencyChecklistScreen: React.FC<EmergencyChecklistScreenProps> = ({ route }) => {
  // Alle Hooks immer am Anfang aufrufen!
  ...
  // Übersetzte Daten laden, falls vorhanden
  const translatedEmergency = t(`emergencies.${emergencyId}`, { returnObjects: true }) as { title?: string, steps?: string[] };
      ...
      <View style={[styles.card, { marginBottom: 16 }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          ...
          <Text style={[styles.itemTitle, { fontSize: baseFontSize * fontSizeScale * 1.3 }]}>
            {translatedEmergency.title || checklistData.title}
          </Text>
        </View>
        <Text style={styles.itemSubtitle}>{t('emergency_checklist_hint')}</Text>
      </View>
    );
  };

  const renderChecklistItem = ({ item, index }: { item: EmergencyStep, index: number }) => {
    const isCompleted = completedSteps.has(item.id);
    // Schritttext übersetzt, falls vorhanden
    let stepText = item.text;
    if (translatedEmergency.steps && translatedEmergency.steps[index]) {
      stepText = translatedEmergency.steps[index];
    }
    return (
      <TouchableOpacity
        style={[
  ...
  if (!checklistData) {
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>{t('emergency_not_found')}</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item, index }) => renderChecklistItem({ item, index })}
...
const EmergencyOverview: React.FC<EmergencyOverviewProps> = ({ navigation, theme, fontSizeScale, baseFontSize, styles }) => {
  const { t } = useTranslation();
  const emergencyItems = Object.values(emergencyData);

  const getEmergencyIconColor = (id: EmergencyId): string => {
    const categoryColors: Record<string, string> = {
      atemnot: theme === 'dark' ? '#64b5f6' : '#2196f3',
      brustschmerz: theme === 'dark' ? '#e57373' : '#f44336',
      hypoglykaemie: theme === 'dark' ? '#ffb74d' : '#ff9800',
      sturz: theme === 'dark' ? '#90a4ae' : '#607d8b',
      fieber_sepsis: theme === 'dark' ? '#f06292' : '#e91e63',
      allergie_anaphylaxie: theme === 'dark' ? '#ba68c8' : '#9c27b0',
      schlaganfallverdacht: theme === 'dark' ? '#4db6ac' : '#009688',
      dehydratation: theme === 'dark' ? '#4fc3f7' : '#03a9f4',
      synkope_kollaps: theme === 'dark' ? '#9575cd' : '#673ab7',
      lungenembolie_verdacht: theme === 'dark' ? '#f48fb1' : '#d81b60',
    };
    return categoryColors[id] || (theme === 'dark' ? '#aaaaaa' : '#555555');
  };

  const renderEmergencyButton = (item: EmergencyChecklistData, t: any) => {
    const iconColor = getEmergencyIconColor(item.id);
    const isEmoji = /\p{Emoji}/u.test(item.icon);
    const translated = t(`emergencies.${item.id}`, { returnObjects: true }) as { title?: string };
    return (
      <TouchableOpacity
        style={[
          styles.card,
